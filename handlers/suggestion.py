"""'💬 Taklif va murojaat' (Suggestion/appeal) FSM flow."""
...
from keyboards.reply import confirm_keyboard, evidence_keyboard, main_menu_keyboard
from services.notify import notify_admin
from states.states import SuggestionStates
from utils.logger import log_user_action
from utils.texts import labels, t
from utils.validators import sanitize_text, validate_file_size
...
@router.message(F.text.in_(labels("menu_suggestion")))
async def start_suggestion(message: Message, state: FSMContext, user_language: str) -> None:
    log_user_action(message.from_user.id, "suggestion_flow_start")
    await state.clear()
    await state.set_data({"attachments": []})
    await state.set_state(SuggestionStates.waiting_message)
    await message.answer(t("ask_suggestion_text", user_language))


@router.message(SuggestionStates.waiting_message, F.text)
async def receive_suggestion_text(message: Message, state: FSMContext, user_language: str) -> None:
    await state.update_data(message_text=sanitize_text(message.text))
    await state.set_state(SuggestionStates.waiting_evidence)
...
@router.message(SuggestionStates.waiting_evidence, F.photo)
...
@router.message(SuggestionStates.waiting_evidence, F.video)
...
@router.message(SuggestionStates.waiting_evidence, F.document)
...
@router.message(SuggestionStates.waiting_evidence, F.voice)
...
@router.message(SuggestionStates.waiting_evidence, F.text.in_(_EVIDENCE_HINT_LABELS))
async def evidence_hint(message: Message, user_language: str) -> None:
    await message.answer(t("ask_evidence", user_language), reply_markup=evidence_keyboard(user_language))


@router.message(SuggestionStates.waiting_evidence, F.text.in_(labels("btn_skip")))
async def finish_evidence(message: Message, state: FSMContext, user_language: str) -> None:
    await state.set_state(SuggestionStates.confirming)
    await message.answer(t("ask_send_confirm", user_language), reply_markup=confirm_keyboard(user_language))


@router.message(SuggestionStates.confirming, F.text.in_(labels("btn_cancel")))
async def cancel_suggestion(message: Message, state: FSMContext, user_language: str) -> None:
    log_user_action(message.from_user.id, "suggestion_cancelled")
    await state.clear()
    await message.answer(t("report_cancelled", user_language), reply_markup=main_menu_keyboard(user_language))


@router.message(SuggestionStates.confirming, F.text.in_(labels("btn_send")))
async def send_suggestion(message: Message, state: FSMContext, user_language: str, bot: Bot) -> None:
    ...
    log_user_action(message.from_user.id, f"suggestion_sent:{report.report_code}")
    await state.clear()
    await message.answer(
        t("suggestion_sent", user_language, code=report.report_code),
